...
async def test_build_llm_client_accepts_api_model_override():
    ...
    client = build_llm_client(settings, api_model="deepseek/deepseek-v4-flash")
    try:
        assert isinstance(client, ApiLlmClient)
...
async def test_build_planner_llm_client_uses_separate_api_model_when_configured():
    ...
    artifact_client = build_llm_client(settings)
    planner_client = build_planner_llm_client(settings, artifact_client)
    try:
