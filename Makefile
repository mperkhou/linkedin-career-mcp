...
SKILL_NAME ?=
SKILL_NAMES ?= $(if $(SKILL_NAME),$(SKILL_NAME),linkedin-career-mcp master-resume-yaml manual-resume-passthrough agentic-workflow-init agentic-workflow-controller)
...
highlight-draft-resumes: venv
	...
	filter_args=""; \
	if [ -n "$(HIGHLIGHT_RESUME_VARIANT)" ]; then \
		filter_args="$$filter_args --variant-key $(HIGHLIGHT_RESUME_VARIANT)"; \
	fi; \
